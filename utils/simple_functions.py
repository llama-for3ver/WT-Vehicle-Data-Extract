...
def traverse_shop(
    shop, lookup_key: str = None, lookup_attribute: str = "marketplaceItemdefId"
) -> bool:
            ...
            if key != "image" and key != "reqAir":
                if "_group" not in key:
                    try:
                        if (
                            key == lookup_key
                            and value_from_dict(shop[key], lookup_attribute) is not None
                        ):
                            return True
                    except KeyError:
                        pass
...
def is_squadron_vehicle(shop, identifier, country, vehicle_type) -> bool:
    country_key = "country_" + country
    type_key = get_type_key(vehicle_type)
    return traverse_shop(
        shop[country_key][type_key]["range"], identifier, "isClanVehicle"
    )
