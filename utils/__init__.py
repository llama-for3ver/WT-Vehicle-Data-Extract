...
from utils.update_localization import (
    ALL_WEAPONS,
    ALL_AMMOS,
    ALL_EXPLOSIVES,
    ALL_AMMO_TYPES,
...
def create_vehicle(v_name: str, v_fetch_path: str) -> Vehicle | None:
    ...
    data.presets = create_presets(
        details, data.has_customizable_weapons, len(data.weapons) > 0
    )
    data.customizable_presets = (
        create_customizable_presets(details, len(data.weapons) > 0)
        if data.has_customizable_weapons
        else None
    )
    return data
...
def create_vehicle_data(
    v_name: str, v_details: dict, v_fetch_path, vehicle_type: str | None
) -> Vehicle | None:
    ...
    """
    vehicle_wiki = value_from_dict(
        v_details, "wiki", value_from_dict(v_details, "Wiki")
    )
    vehicle_data = value_from_dict(WPCOST, v_name)
    vehicle_tags = value_from_dict(value_from_dict(UNIT_TAGS, v_name), "Shop")
    vehicle_phys = value_from_dict(
        v_details, "VehiclePhys", value_from_dict(v_details, "ShipPhys")
    )

    if v_name not in WPCOST or (
        vehicle_type is not None
        and value_from_dict(vehicle_data, "unitMoveType") != vehicle_type
    ):
        cLogger.error(
    ...
    country = (
        country.replace("country_", "").lower() if country is not None else country
    )
    ...
    types = [
        x.replace("type_", "").lower()
        for x in tags.keys()
        if (x != "boat" or x != "ship") and x.startswith("type_")
    ]
    ...
    vehicle.release_date = value_from_dict(
        value_from_dict(UNIT_TAGS, v_name), "releaseDate", None
    )
    ...
    vehicle.arcade_br = BATTLE_RATINGS[
        value_from_dict(
            vehicle_data,
            "economiceraArcade",
            value_from_dict(vehicle_data, "economicRankArcade", 1.0),
        )
    ]
    vehicle.realistic_br = BATTLE_RATINGS[
        value_from_dict(
            vehicle_data,
            "economiceraHistorical",
            value_from_dict(vehicle_data, "economicRankHistorical", 1.0),
        )
    ]
    vehicle.simulator_br = BATTLE_RATINGS[
        value_from_dict(
            vehicle_data,
            "economiceraSimulation",
            value_from_dict(vehicle_data, "economicRankSimulation", 1.0),
        )
    ]
    economic_rank_tank_historical = value_from_dict(
        vehicle_data, "economicRankTankHistorical"
    )
    vehicle.realistic_ground_br = (
        BATTLE_RATINGS[economic_rank_tank_historical]
        if economic_rank_tank_historical is not None
        else vehicle.realistic_br
    )
    economic_rank_simulation = value_from_dict(
        vehicle_data,
        "economiceraSimulation",
        value_from_dict(vehicle_data, "economicRankSimulation", 1.0),
    )
    vehicle.simulator_ground_br = (
        BATTLE_RATINGS[economic_rank_simulation]
        if economic_rank_simulation is not None
        else vehicle.simulator_br
    )

    vehicle.value = value_from_dict(vehicle_data, "value", 0)
    vehicle.req_exp = value_from_dict(vehicle_data, "reqExp", 0)
    vehicle.is_premium = (
        True if value_from_dict(vehicle_data, "costGold") is not None else False
    )

    vehicle.squadron_vehicle = is_squadron_vehicle(
        SHOP, vehicle.identifier, vehicle.country, vehicle.vehicle_type
    )
    vehicle.on_marketplace = is_vehicle_on_marketplace(
        SHOP, vehicle.identifier, vehicle.country, vehicle.vehicle_type
    )
    vehicle.is_pack = (
        False
        if (
            vehicle.on_marketplace or vehicle.squadron_vehicle or not vehicle.is_premium
        )
        else is_pack(SHOP, vehicle.identifier, vehicle.country, vehicle.vehicle_type)
    )
    ...
    vehicle_phys_mass = value_from_dict(
        vehicle_phys, "Mass", value_from_dict(vehicle_phys, "mass")
    )
    empty_mass = value_from_dict(vehicle_phys_mass, "Empty", 0.0)
    fuel_mass = value_from_dict(vehicle_phys_mass, "Fuel", 0.0)
    vehicle_wiki_general = value_from_dict(
        vehicle_wiki, "general", value_from_dict(vehicle_wiki, "General")
    )
    ...
    vehicle.repair_time_realistic = value_from_dict(
        vehicle_data, "repairTimeHrsHistorical", 0
    )
    vehicle.repair_time_simulator = value_from_dict(
        vehicle_data, "repairTimeHrsSimulation", 0
    )
    vehicle.repair_time_no_crew_arcade = value_from_dict(
        vehicle_data, "repairTimeHrsNoCrewArcade", 0
    )
    vehicle.repair_time_no_crew_realistic = value_from_dict(
        vehicle_data, "repairTimeHrsNoCrewHistorical", 0
    )
    vehicle.repair_time_no_crew_simulator = value_from_dict(
        vehicle_data, "repairTimeHrsNoCrewSimulation", 0
    )
    vehicle.repair_cost_arcade = value_from_dict(vehicle_data, "repairCostArcade", 0)
    vehicle.repair_cost_realistic = value_from_dict(
        vehicle_data, "repairCostHistorical", 0
    )
    vehicle.repair_cost_simulator = value_from_dict(
        vehicle_data, "repairCostSimulation", 0
    )
    vehicle.repair_cost_per_min_arcade = value_from_dict(
        vehicle_data, "repairCostPerMinArcade", 0
    )
    vehicle.repair_cost_per_min_realistic = value_from_dict(
        vehicle_data, "repairCostPerMinHistorical", 0
    )
    vehicle.repair_cost_per_min_simulator = value_from_dict(
        vehicle_data, "repairCostPerMinSimulation", 0
    )
    vehicle.repair_cost_full_upgraded_arcade = value_from_dict(
        vehicle_data, "repairCostFullUpgradedArcade", 0
    )
    vehicle.repair_cost_full_upgraded_realistic = value_from_dict(
        vehicle_data, "repairCostFullUpgradedHistorical", 0
    )
    vehicle.repair_cost_full_upgraded_simulator = value_from_dict(
        vehicle_data, "repairCostFullUpgradedSimulation", 0
    )
    vehicle.required_vehicle = value_from_dict(vehicle_data, "reqAir", None)

    vehicle.engine = create_vehicle_data_engine(
        v_fetch_path, vehicle_phys, vehicle_tags
    )
    vehicle.aerodynamics = (
        create_vehicle_data_aerodynamics(vehicle_wiki, vehicle_tags)
        if vehicle.vehicle_type in AIR_TYPES
        else None
    )
    ...
    vehicle.ballistic_computer = create_vehicle_ballistic_computer(
        v_details, vehicle.vehicle_type
    )
    return vehicle


def create_vehicle_data_engine(
    v_fetch_path: str, vehicle_phys: dict, vehicle_tags: dict
) -> Engine:
    ...
    if "ships" in v_fetch_path:
        ...
        max_rev_speed = value_from_dict(
            engine, "maxRevSpeed", value_from_dict(engine, "maxReverseSpeed", 0)
        )
        final_engine.max_speed_rb_sb = floor(
            max_speed * 3.6 if type(max_speed) is float else max_speed[0] * 3.6
        )
        final_engine.max_reverse_speed_rb_sb = floor(
            max_rev_speed * 3.6
            if isinstance(max_rev_speed, Number)
            else max_rev_speed[0] * 3.6
        )
        final_engine.max_speed_ab = proper_round(
            final_engine.max_speed_rb_sb * ENGINE_SPEED_AB_MUL_SHIP
        )
        final_engine.max_reverse_speed_ab = proper_round(
            final_engine.max_reverse_speed_rb_sb * ENGINE_SPEED_AB_MUL_SHIP
        )

    elif "flightmodels" in v_fetch_path:
        final_engine.max_speed_rb_sb = floor(
            value_from_dict(vehicle_tags, "maxSpeed", 0) * 3.6
        )
        final_engine.max_reverse_speed_rb_sb = proper_round(
            final_engine.max_speed_rb_sb * ENGINE_SPEED_AB_MUL_AIR
        )

    elif "tankmodels" in v_fetch_path:
        engine = value_from_dict(vehicle_phys, "engine")
        mechanics = value_from_dict(
            vehicle_phys, "mechanics", value_from_dict(vehicle_phys, "Mechanics")
        )
        ...
        final_engine.horse_power_ab = proper_round(
            final_engine.horse_power_rb_sb * ENGINE_HP_AB_MUL_TANK
        )

        try:
            final_engine.max_speed_rb_sb = floor(
                (
                    (final_engine.max_rpm * drive_gear_radius)
                    / (main_gear_ratio * side_gear_ratio * gears[-1])
                )
                * 0.12
                * 3.14
            )
            final_engine.max_reverse_speed_rb_sb = floor(
                (
                    (final_engine.max_rpm * drive_gear_radius)
                    / (main_gear_ratio * side_gear_ratio * gears[0])
                )
                * 0.12
                * 3.14
            )
        ...
        final_engine.max_speed_ab = int(
            proper_round(final_engine.max_speed_rb_sb * ENGINE_SPEED_AB_MUL_TANK)
        )
        final_engine.max_reverse_speed_ab = int(
            proper_round(
                final_engine.max_reverse_speed_rb_sb * ENGINE_SPEED_AB_MUL_TANK
            )
        )
    return final_engine


def create_vehicle_data_aerodynamics(
    vehicle_wiki: dict, vehicle_tags: dict
) -> Aerodynamics:
    ...
    aerodynamics.runway_length_required = value_from_dict(
        vehicle_tags, "airfieldLen", 0
    )
...
def create_vehicle_data_modifications(vehicle_wpcost: dict) -> set[Modification]:
    ...
    for mod_name in modifications.keys():
        ...
        modification.ge_cost = (
            value_from_dict(mod, "costGold", 0)
            if value_from_dict(mod, "openCostGold") is None
            else value_from_dict(mod, "openCostGold", 0)
        )
        ...
        if mod_dict is not None:
            ...
            if modification.icon is not None:
                modification.icon = modification.icon.replace(
                    "#ui/gameuiskin#", "modifications/"
                )
...
def create_vehicle_night_vision(
    v_details: dict, vehicle_type: str
) -> tuple[NightVisionDevice, NightVisionDevice] | tuple[None, None]:
    ...
    if vehicle_type in SEA_TYPES2:
        ...
        if (
            ...
            or "heli_night_vision_system" not in modifications.keys()
        ):
            return None, None
        night_vision_modification = value_from_dict(
            modifications, "heli_night_vision_system"
        )
    elif vehicle_type in GROUND_TYPES2:
        if modifications is None or "night_vision_system" not in modifications.keys():
            return None, None
        night_vision_modification = value_from_dict(
            modifications, "night_vision_system"
        )
    ...
    for device in night_vision_devices:
        if device in device_mapping:
            ...
            resolution = tuple(
                value_from_dict(night_vision_devices.get(device), "resolution")
            )
...
def create_vehicle_ballistic_computer(
    v_details: dict, vehicle_type: str
) -> BallisticComputer | None:
    ...
    if vehicle_type in SEA_TYPES or vehicle_type in GROUND_TYPES:
        ...
        ballistic_computer.has_gun_ccip = bool(
            value_from_dict(v_details, "haveCCIPForGun", False)
        )
        ballistic_computer.has_turret_ccip = bool(
            value_from_dict(v_details, "haveCCIPForTurret", False)
        )
        ballistic_computer.has_bombs_ccip = bool(
            value_from_dict(v_details, "haveCCIPForBombs", False)
        )
        ballistic_computer.has_rocket_ccip = bool(
            value_from_dict(v_details, "haveCCIPForRocket", False)
        )
        ballistic_computer.has_gun_ccrp = bool(
            value_from_dict(v_details, "haveCCRPForGun", False)
        )
        ballistic_computer.has_turret_ccrp = bool(
            value_from_dict(v_details, "haveCCRPForTurret", False)
        )
        ballistic_computer.has_bombs_ccrp = bool(
            value_from_dict(v_details, "haveCCRPForBombs", False)
        )
        ballistic_computer.has_rocket_ccrp = bool(
            value_from_dict(v_details, "haveCCRPForRocket", False)
        )
...
def create_weapons(v_name: str, v_details: dict, customizable: bool = False):
    ...
    if customizable:
        if value_from_dict(v_details, "commonWeapons") == {}:
            return final_weapons
        default_weapons = value_from_dict(
            value_from_dict(v_details, "WeaponSlots"), "WeaponSlot", []
        )
                    ...
                    for gun_part in value_from_dict(gun, "Weapon"):
                        if "dummy" not in gun_part["blk"]:
                            preset.weapons.append(
                                create_weapon_details(gun_part["blk"])
                            )
        ...
        # For when vehicles can mount only one weapon (F4U-4)
            ...
            if isinstance(
                value_from_dict(default_weapons, "WeaponPreset", [])["Weapon"], list
            ):
                for gun in value_from_dict(default_weapons, "WeaponPreset", [])[
                    "Weapon"
                ]:
        ...
        vehicle_file_weapons = value_from_dict(
            value_from_dict(v_details, "commonWeapons"), "Weapon", []
        )
        vehicle_file_weapons = (
            [vehicle_file_weapons]
            if isinstance(vehicle_file_weapons, dict)
            else vehicle_file_weapons
        )
        ...
        for weapon in vehicle_file_weapons:
            ...
            if weapon_path is None:
                ...
                continue
            weapon_path = (
                weapon_path if weapon_path.endswith(".blk") else f"{weapon_path}.blk"
            )
...
def create_weapon_details(weapon_path: str, count: int = 1):
    ...
    is_rocket: bool = (
        value_from_dict(weapon_blkx, ROCKET_NAME)
        or dictHasKeyInsensitive(weapon_blkx, ROCKET_TYPE)
        or value_from_dict(weapon_blkx, ROCKET_TYPE)
    )
    is_cannon: bool = (
        value_from_dict(weapon_blkx, CANNON_NAME)
        or dictHasKeyInsensitive(weapon_blkx, CANNON_TYPE)
        or value_from_dict(weapon_blkx, CANNON_TYPE)
        or weapon_type == -1
        or weapon_type == 1
        or weapon_type == 3
    )
    is_torpedo: bool = (
        value_from_dict(weapon_blkx, TORPEDO_NAME)
        or dictHasKeyInsensitive(weapon_blkx, TORPEDO_TYPE)
        or value_from_dict(weapon_blkx, TORPEDO_TYPE)
        or weapon_type == 1
    )
    is_bomb: bool = (
        value_from_dict(weapon_blkx, BOMB_NAME)
        or dictHasKeyInsensitive(weapon_blkx, BOMB_TYPE)
        or value_from_dict(weapon_blkx, BOMB_TYPE)
    )
    is_booster: bool = (
        value_from_dict(weapon_blkx, BOOSTER_NAME)
        or dictHasKeyInsensitive(weapon_blkx, BOOSTER_TYPE)
        or value_from_dict(weapon_blkx, BOOSTER_TYPE)
    )
    is_container: bool = (
        value_from_dict(weapon_blkx, CONTAINER_NAME)
        or dictHasKeyInsensitive(weapon_blkx, CONTAINER_TYPE)
        or value_from_dict(weapon_blkx, CONTAINER_TYPE)
    )
    is_extfueltank: bool = (
        value_from_dict(weapon_blkx, EXTFUELTANK_NAME)
        or dictHasKeyInsensitive(weapon_blkx, EXTFUELTANK_TYPE)
        or value_from_dict(weapon_blkx, EXTFUELTANK_TYPE)
    )

    valid = (
        weapon_type is not None
        or is_rocket
        or is_torpedo
        or is_cannon
        or is_bomb
    ...
    if not valid:
        cLogger.error(
            f"""The weapon {name_regex[0]} -  type is missing {get_guns_url(weapon_path)} weapon_type={weapon_type}\tis_rocket={is_rocket}\tis_cannon={is_cannon}\tis_torpedo={is_torpedo}\tIS_BOMB={is_bomb}\tis_container={is_container}"""
        )
        return
        ...
        if is_cannon
            ...
            if is_rocket
                ...
                if is_torpedo
                    ...
                    BOMB_NAME
                    if is_bomb
                    else BOOSTER_NAME if is_booster else CONTAINER_NAME
                )
            )
        )
    )
...
def get_ammos_by_weapon(weapon_type: str, all_weapons: dict[any], all_weapons_keys=[]):
    ...
    for key in all_weapons_keys:
        ...
        if (
            isinstance(key_value, dict) or isinstance(key_value, list)
        ) and key.lower() == weapon_type.lower():
            raw_ammo = key_value
        elif isinstance(key_value, dict) and dictHasKeyInsensitive(
            key_value, weapon_type
        ):
...
def create_ammo(raw_ammo: dict):
    ...
    name = value_from_dict(
        raw_ammo, "bombName", value_from_dict(raw_ammo, "bulletName")
    )
    _type = value_from_dict(
        raw_ammo, "bombType", value_from_dict(raw_ammo, "bulletType")
    )
    mass = value_from_dict(raw_ammo, "mass")
    mass = mass[0] if type(mass) is list else mass
    # ship else generic
    speed = value_from_dict(
        raw_ammo,
        "maxSpeedInWater",
    ...
    # ship else generic
    max_distance = value_from_dict(
        raw_ammo, "distToLive", value_from_dict(raw_ammo, "maxDistance")
    )
...
def create_presets(v_details: dict, customizable: bool, has_offensive_weapons: bool):
    ...
    if customizable:
        ...
        for preset in presets[1:] if has_offensive_weapons else presets:
            ...
            preset_details = value_from_dict(
                myFetch(get_guns_url(preset["blk"]), True), "Weapon", []
            )
            ...
            for weapon_of_preset in preset_details:
                ...
                pylon_guns = value_from_dict(
                    weapon_slot[SLOT_HASHMAP[weapon_of_preset["slot"]]], "WeaponPreset"
                )
                ...
                if isinstance(pylon_guns, dict):
                    if type(pylon_guns["Weapon"]) is list:
                        for weapon in pylon_guns["Weapon"]:
                            if "dummy" not in value_from_dict(weapon, "blk", []):
                                final_preset.weapons.append(
                                    create_weapon_details(weapon["blk"])
                                )
                    else:
                        if "dummy" not in value_from_dict(
                            pylon_guns["Weapon"], "blk", []
                        ):
                            final_preset.weapons.append(
                                create_weapon_details(pylon_guns["Weapon"]["blk"])
                            )
                    ...
                    for weapon in pylon_guns:
                        ...
                        if weapon["name"] == weapon_of_preset["preset"]:
                            if isinstance(weapon["Weapon"], dict) and (
                                "dummy" not in value_from_dict(weapon["Weapon"], "blk")
                            ):
                                final_preset.weapons.append(
                                    create_weapon_details(weapon["Weapon"]["blk"])
                                )
                            # TODO: Find an example for this?
        ...
        for preset in presets:
            ...
            if (
                "_default" not in blk.lower()
                and "empty" not in blk.lower()
                and preset.get("Weapon") is not None
            ):
                final_preset = Preset()
                ...
                if type(weapons) is not list:
                    if "dummy" not in value_from_dict(preset["Weapon"], "blk", []):
                        final_preset.weapons.append(
                            create_weapon_details(
                                value_from_dict(preset["Weapon"], "blk")
                            )
                        )
                else:
                    for weapon in weapons:
                        if "dummy" not in value_from_dict(preset["Weapon"], "blk", []):
                            final_preset.weapons.append(
                                create_weapon_details(value_from_dict(weapon, "blk"))
                            )
...
def create_customizable_presets(
    v_details: dict, has_offensive_weapons: bool
) -> CustomizablePreset:
    weapon_slots = value_from_dict(v_details, "WeaponSlots")

    customizable_preset = CustomizablePreset()
    customizable_preset.max_load = value_from_dict(weapon_slots, "maxloadMass", 0)
    customizable_preset.max_load_left_wing = value_from_dict(
        weapon_slots, "maxloadMassLeftConsoles", 0
    )
    customizable_preset.max_load_right_wing = value_from_dict(
        weapon_slots, "maxloadMassRightConsoles", 0
    )
    customizable_preset.max_disbalance = value_from_dict(
        weapon_slots, "maxDisbalance", 0
    )
    ...
    for slot in weapon_slot[1:] if has_offensive_weapons else weapon_slot:
        ...
        pylon.used_for_disbalance = not value_from_dict(
            slot, "notUseforDisbalanceCalculation", False
        )
            ...
            if weapons is not None:
                for weapon in weapons:
                    ...
                    if "dummy_weapon" not in weapon["blk"]:
                        pylon.selectable_weapons.append(
                            create_weapon_details(weapon["blk"])
                        )
